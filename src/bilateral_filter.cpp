...
int main(int argc, char** argv)
{
    ...
    cout << "Input depth range: [" << minV << ", " << maxV << "]\n";
    ...
    // 应用双边滤波器（如果需要则迭代应用）
    Mat current = depth32f;
    for (int i = 0; i < iterations; ++i) {
        Mat out;
        bilateralFilter(current, out, diameter, sigmaColor, sigmaSpace);
        current = out; 
    }
    Mat filtered = current;
    ...
    cout << "Params: d=" << diameter << ", sigmaColor=" << sigmaColor
        ...
        Mat visFloat, vis8u;
        normalize(filtered, visFloat, 0, 255, NORM_MINMAX);
        visFloat.convertTo(vis8u, CV_8U);
        String vis_path = out_path + ".png";
        imwrite(vis_path, vis8u);

    

    Mat visNorm;
    normalize(filtered, visNorm, 0, 1, NORM_MINMAX);
    flip(visNorm, visNorm, -1);
    imshow("filtered ", visNorm);
    waitKey(0);
    return 0;
}
